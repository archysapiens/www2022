...
$sql = "select 'Servidor Público' as serpub, b.cr, b.ur, b.puesto ,
...
and b.QNAPROC='12' and b.tipotrab!='20'";
		...
		if ($NombreEmpleado!="XXXXXXX"){
			...
			if($PrestacionesEconomicas > 0){
			 $sqlIN = "select bt.importe, pr.CONCEPTO, pr.PARTIDA_ANTECEDENTE,  pr.DENOMINACION
						from btac2016 bt, partida_remu pr
						where bt.tconcep=1 and  
						bt.CONCEP=pr.CONCEPTO and bt.ptaant = pr.PARTIDA_ANTECEDENTE and 
						bt.CONCEP in('59','73') and bt.PTAANT in ('DT','DR','DM')
						and bt.llave=:llave";
	        	$res = $db->execFetchAll($sqlIN, "Query Example", array(array(":llave", $LLAVE, -1)));
...
/**
		$Str =$Contador."|".$UR."|".$Puesto."|".$NombreEmpleado."|".$ApePaterno.
				"|".$ApePaterno."|". $PER."|".$NETO."|".$TIPOTRAB."|".$Gratificaciones.
				"|".$Estimulos."|". $PeriodicidadEstimulo."|".$ApoyoEconomico."|".$QNAPROC.
