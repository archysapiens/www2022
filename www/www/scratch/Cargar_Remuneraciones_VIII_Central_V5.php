...
//$ArrURS = array( "UR"); // para enviar en un solo archivo todas las UR's
...
foreach ($Trimestres as $Trim) { 
	...
	foreach ($ArrURS as $urs) { 
		...
		switch($urs){
			...
			case "UR": $Condicion  = "bd.ur =bd.ur"; break;
		...
		if (!$GestorArchivo) {
			...
			//	echo "id uNIDAD >". $row['UNIDADES_ID'] ."<< <BR>";
				...
				$Base = (int) $row['BASE'];
				//echo " Base >>$Base<< <br>";
				$DetallePrestaciones="\n Servidor publico No Obtuvo Pretacion de Reyes, Madres o dia del Servidor Publico";
				$ImportePrestaciones =CERO;
				if( $Base > 0){
						...
						foreach ($Resultado as $rowBase) {
							...
							if(in_array($TIPNOM, $ArrTipNom)){
								if (isset($ArrContratos[$DESCRIPCION]))
									$ArrContratos[$DESCRIPCION]	= $ArrContratos[$DESCRIPCION]." Quincena Pago:".$QNAENVIO." Monto Prestacion: $".$IMPORTEBASE;
								else {
										$ArrContratos[$DESCRIPCION]	= " Quincena Pago:".$QNAENVIO." Monto Prestacion: $".$IMPORTEBASE;
									}	
...
//		            ->setCellValue('P'.$Contador, $Contador-SIETE)
				...
				->setCellValue('AM'.$Contador,$DetallePrestaciones);                        
